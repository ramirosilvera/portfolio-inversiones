...
describe('alertasBonos', () => {
  ...
  // 100 años máximo de duración = prácticamente deshabilitado, 0% mínimo ley extranjera =
  // prácticamente deshabilitado, ya que es un umbral MÍNIMO) — los tests que apuntan a alguna de
  // estas 3 condiciones pasan umbrales ajustados explícitos.
  const MIN_GRADO = 50, MAX_DURACION = 100, MIN_LEY_EXTRANJERA = 0;
  ...
  it('cartera balanceada, sin bonos especulativos/default, duración corta: sin alertas', () => {
    ...
    const r = resumenBonos([a, b, c]);
    expect(alertasBonos(r, MIN_GRADO, MAX_DURACION, MIN_LEY_EXTRANJERA)).toHaveLength(0);
  });
  ...
  it('mayor posición ≥40%: alerta warn', () => {
    ...
    expect(alertasBonos(r, MIN_GRADO, MAX_DURACION, MIN_LEY_EXTRANJERA).some(a => a.severidad === 'warn' && a.texto.includes('AL30'))).toBe(true);
  });

  it('capital en default: alerta neg (severidad alta)', () => {
    const enDefault = conRating('DEF1', 100, 'S&P', 'D');
    const r = resumenBonos([enDefault]);
    const alertas = alertasBonos(r, MIN_GRADO, MAX_DURACION, MIN_LEY_EXTRANJERA);
  ...
  it('≥40% especulativo: alerta warn', () => {
    ...
    expect(alertasBonos(r, MIN_GRADO, MAX_DURACION, MIN_LEY_EXTRANJERA).some(a => a.texto.includes('especulativo'))).toBe(true);
  });
  ...
  it('grado de inversión por debajo del mínimo personal: alerta warn', () => {
    ...
    const alertas = alertasBonos(r, 60, MAX_DURACION, MIN_LEY_EXTRANJERA); // mínimo personal 60% > 40% real
    ...
    expect(alertasBonos(r, 30, MAX_DURACION, MIN_LEY_EXTRANJERA).some(a => a.texto.includes('mínimo personal'))).toBe(false);
  });
  ...
  it('duración promedio por encima del máximo personal: alerta warn', () => {
    ...
    const alertas = alertasBonos(r, MIN_GRADO, 2, MIN_LEY_EXTRANJERA); // máximo personal 2 años, muy por debajo de la duración real
    ...
    expect(alertasBonos(r, MIN_GRADO, 100, MIN_LEY_EXTRANJERA).some(a => a.texto.includes('máximo personal'))).toBe(false);
  });

  it('% ley extranjera por debajo del mínimo personal: alerta warn (umbral MÍNIMO, no máximo — ley extranjera es la opción más segura)', () => {
    const local = calcularBono({ ...basePos, ticker: 'AL30', ley: 'local' }, 1, HOY);           // capitalUsado 1000
    const extranjera = calcularBono({ ...basePos, ticker: 'GD30', cantidad: 500, ley: 'extranjera' }, 1, HOY); // capitalUsado 500
    const r = resumenBonos([local, extranjera]); // 500/1500 ≈ 33% ley extranjera
    const alertas = alertasBonos(r, MIN_GRADO, MAX_DURACION, 60); // mínimo personal 60% (default de la app) > 33% real
    expect(alertas.some(a => a.severidad === 'warn' && a.texto.includes('ley extranjera') && a.texto.includes('mínimo personal'))).toBe(true);
    // con un mínimo personal más laxo (20% < 33% real), la misma cartera no alerta por este motivo
    expect(alertasBonos(r, MIN_GRADO, MAX_DURACION, 20).some(a => a.texto.includes('ley extranjera'))).toBe(false);
  });
  ...
  it('spread negativo (TIR < tasa libre de riesgo): alerta neg', () => {
    ...
    expect(alertasBonos(r, MIN_GRADO, MAX_DURACION, MIN_LEY_EXTRANJERA).some(a => a.severidad === 'neg' && a.texto.includes('spread'))).toBe(true);
  });

  it('bonos sin datos: alerta warn', () => {
    const incompleto = calcularBono({ ...basePos, ticker: 'XXX', cupon_tasa: null }, 0.6, HOY);
    const r = resumenBonos([incompleto]);
    expect(alertasBonos(r, MIN_GRADO, MAX_DURACION, MIN_LEY_EXTRANJERA).some(a => a.texto.includes('sin cupón o vencimiento'))).toBe(true);
  });

  it('bono marcado amortizable sin valor residual cargado: alerta warn', () => {
    const sinVR = calcularBono({ ...basePos, ticker: 'XXX', amortizable: true, valor_residual: null }, 0.6, HOY);
    const r = resumenBonos([sinVR]);
    expect(alertasBonos(r, MIN_GRADO, MAX_DURACION, MIN_LEY_EXTRANJERA).some(a => a.texto.includes('amortizable') && a.texto.includes('valor residual'))).toBe(true);
  });

  it('bono amortizable CON valor residual cargado: no dispara esa alerta', () => {
    const conVR = calcularBono({ ...basePos, ticker: 'YYY', amortizable: true, valor_residual: 0.7 }, 0.6, HOY);
    const r = resumenBonos([conVR]);
    expect(alertasBonos(r, MIN_GRADO, MAX_DURACION, MIN_LEY_EXTRANJERA).some(a => a.texto.includes('valor residual'))).toBe(false);
  });

  it('cartera vacía: sin alertas, no revienta', () => {
    expect(alertasBonos(resumenBonos([]), MIN_GRADO, MAX_DURACION, MIN_LEY_EXTRANJERA)).toHaveLength(0);
  });
});
